...
module(procs_spec, {
    ...
    it("parses parameterized procs and procs returning literals", {
        ...
        parse("{ x = 2 } eval", vector_new(FM_START_PROC, FM_INTEGER, string_new("2"), FM_STORE_LOCAL, string_new("x"), FM_END_PROC, FM_UNARY, string_new("eval")));
        parse("{ param | param puts } value: 42", vector_new(FM_START_PROC, FM_PROC_PARAMETER, string_new("param"), FM_LOCAL, string_new("param"), FM_UNARY, string_new("puts"), FM_END_PROC, FM_INTEGER, string_new("42"), FM_KEYWORD, string_new("value:"), string_new("1")));
    });
})
