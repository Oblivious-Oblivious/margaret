...
VM *emitter_emit(VM *vm, vector *formal_bytecode) {
    ...
    MargMethod *main_method = AS_METHOD(table_get(&marg_object->messages, MARG_STRING("")));
    ...
    for(size_t ip = 0; ip < bytecode_size; ip++) {
        ...
        opcode_case(FM_KEYWORD) {
            ...
            keyword_case("primitive_messages:")
                emit_byte(OP_PRIM_MESSAGES);
            keyword_case("primitive_object_id:")
                emit_byte(OP_PRIM_OBJECT_ID);
            keyword_case("primitive_to_string:")
                emit_byte(OP_PRIM_TO_STRING);
            keyword_case("primitive_equals?:with:")
                emit_byte(OP_PRIM_EQUALS);
            keyword_case("primitive_equals_numeric?:with:")
                emit_byte(OP_PRIM_EQUALS_NUMERIC);
            keyword_case("primitive_dnu:msg:")
                emit_byte(OP_PRIM_DNU);
            keyword_case("primitive_clone_object:with_name:")
                emit_byte(OP_PRIM_CLONE_OBJECT);
            keyword_case("primitive_bind_method:to:")
                emit_byte(OP_PRIM_BIND_METHOD);
            keyword_case("primitive_add:with:")
                emit_byte(OP_PRIM_ADD);
            keyword_case("primitive_sub:with:")
                emit_byte(OP_PRIM_SUB);
            keyword_case("primitive_mul:with:")
                emit_byte(OP_PRIM_MUL);
            keyword_case("primitive_div:with:")
                emit_byte(OP_PRIM_DIV);
            keyword_case("primitive_abs:")
                emit_byte(OP_PRIM_ABS);
            keyword_case("primitive_less:than:")
                emit_byte(OP_PRIM_LT);
            keyword_case("primitive_greater:than:")
                emit_byte(OP_PRIM_GT);
            keyword_case("primitive_less_or_equals:than:")
                emit_byte(OP_PRIM_LTE);
            keyword_case("primitive_greater_or_equals:than:")
                emit_byte(OP_PRIM_GTE);
            keyword_case("primitive_incr:")
                emit_byte(OP_PRIM_INCR);
            keyword_case("primitive_decr:")
                emit_byte(OP_PRIM_DECR);
            keyword_case("primitive_double:")
                emit_byte(OP_PRIM_DOUBLE);
