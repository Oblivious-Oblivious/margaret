...
describe Parser do
    context "on associations hashes" do
        it "parses hash literals" do
            ...
            parse("({:a: 1, :b: 2, :c: 3})", %Q{(new Hash (:"a": 1, :"b": 2, :"c": 3))});
            parse("({:a: self, :b: super, :c: 3})", %Q{(new Hash (:"a": self, :"b": super, :"c": 3))});
            parse(%Q{({"k1": "v1", "k2": "v2", "k3": "v3"})}, %Q{(new Hash ("k1": "v1", "k2": "v2", "k3": "v3"))});
            parse("({:a: x, :b: y, :c: z})", %Q{(new Hash (:"a": x, :"b": y, :"c": z))});
        ...
        it "parses associations" do
            parse("(x = Association key: :a value: 100)", %Q{(= x key:value: Association :"a" 100)});
            parse("(y = x key)", "(= y key x)");
            parse("(y = x value)", "(= y value x)");
        end

        it "parses hashes" do
            parse("(x = {:a: 100, :b: 200, :c: 300})", %Q{(= x new Hash (:"a": 100, :"b": 200, :"c": 300))});
            ...
            parse("(b = x includes_key: :a)", %Q{(= b includes_key: x :"a")});
