...
describe Parser do
    context "on lists" do
        ...
        it "parses list messages" do
            ...
            parse("(arr each: ->{ a | a puts })", %Q{(each: arr params:function: Block (a) puts a)});
            parse("(b = arr conform: ->{ a | (a >= 1) && (a <= 4) })", %Q{(= b conform: arr params:function: Block (a) && (>= a 1) (<= a 4))});
            parse("(x = arr select: ->{ a | a > 2 })", %Q{(= x select: arr params:function: Block (a) > a 2)});
            parse("(x = arr reject: ->{ a | a < 2 })", %Q{(= x reject: arr params:function: Block (a) < a 2)});
            parse("(x = arr collect: ->{ a | a + a })", %Q{(= x collect: arr params:function: Block (a) + a a)});
            parse("(x = arr detect: ->{ a | a > 3 } if_none: ())", %Q{(= x detect:if_none: arr params:function: Block (a) > a 3 ())});
            parse("(
                arr = (1, 2, 3, 4),
                sum = 0,
                arr each: ->{ a | sum += a },
                sum
            )", %Q{(= arr (1, 2, 3, 4), = sum 0, each: arr params:function: Block (a) += sum a, sum)});
            parse("(sum = arr inject: 0 into: ->{ a, c | a + c })", %Q{(= sum inject:into: arr 0 params:function: Block (a, c) + a c)});
            parse("(sum = arr fold: 0 into: ->{ a, c | a + c })", %Q{(= sum fold:into: arr 0 params:function: Block (a, c) + a c)});
            parse("(max = arr
                    inject: 0
                    into: -> { a, c |
                        (a > c) if_true: a
            ...
            )", %Q{(= max inject:into: arr 0 params:function: Block (a, c) if_true:if_false: (> a c) a b)});
