...
static table_entry *
table_find_entry(table_entry *entries, size_t capacity, MargValue key) {
  ...
  while(true) {
    ...
    index = MEMORY_GROW_FACTOR == 2 ? (index + 1) & (capacity - 1)
                                    : (index + 1) % capacity;
  }
}
...
static void table_adjust_capacity(table *self, size_t capacity) {
  table_entry *entries =
    (table_entry *)collected_malloc(sizeof(table_entry) * capacity);
...
table_find_string(table *self, char *chars, size_t size, uint64_t hash) {
  ...
  uint64_t index = MEMORY_GROW_FACTOR == 2 ? hash & (self->capacity - 1)
                                           : hash % self->capacity;
  ...
  while(true) {
    ...
    index = MEMORY_GROW_FACTOR == 2 ? (index + 1) & (self->capacity - 1)
