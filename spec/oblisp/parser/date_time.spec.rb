...
describe Parser do
    context "on date and time" do
        it "parses dates" do
            ...
            parse("(x = Date day: 6 month: :july year: 2022)", %Q{(= x (day:month:year: Date 6 (new Symbol "july") 2022))});
            parse("(x = Date from_days: 36000)", "(= x (from_days: Date 36000))");
            parse("(x = Date day_of_week: :Monday)", %Q{(= x (day_of_week: Date (new Symbol "Monday")))}); # 1
            parse("(x = Date index_of_month: :March)", %Q{(= x (index_of_month: Date (new Symbol "March")))}); # 3
