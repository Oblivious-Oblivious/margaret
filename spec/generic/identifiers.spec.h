...
module(identifiers_spec, {
    it("parses identifiers", {
        parse("(ident, 42, another)", vector_new(FM_VARIABLE, string_new("ident"), FM_INTEGER, string_new("42"), FM_VARIABLE, string_new("another")));
        parse("(stuff,ident2)", vector_new(FM_VARIABLE, string_new("stuff"), FM_VARIABLE, string_new("ident2")));
    });

    it("parses normal and instance variables", {
        parse("variable", vector_new(FM_VARIABLE, string_new("variable")));
