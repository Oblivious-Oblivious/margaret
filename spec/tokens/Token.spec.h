...
module(TokenSpec, {
    describe("Token", {
        it("has a type and a value", {
            Token *tok = token_new(marg_string_new("42"), TOKEN_INTEGER, 125);
        ...
        it("has an equals message for values", {
            Token *tok = token_new(marg_string_new("hello"), TOKEN_STRING, 10);
            assert_that(token_equals_values(tok, marg_string_new("hello")));
        });

        it("has an equals message for values", {
            Token *tok1 = token_new(marg_string_new("hello"), TOKEN_STRING, 10);
            Token *tok2 = token_new(marg_string_new("hello"), TOKEN_STRING, 12);
